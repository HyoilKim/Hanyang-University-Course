...
RC IX_BTNode::MergeNode(IX_BTNode * aNode) { //merge할 대상
   ...
   for (int i = 0; i < aNode->GetKeysNum(); i++) {
      void *k;
      aNode->GetKey(i, k);
      RID rid = aNode->GetAddr(i); //aNode의 key와 포인터값을 가져와서 자신의 노드에 inset
      RC rc = InsertNode(k, rid); //성공적으로 insertnode될 경우 0 반환 (sort되어 insert된 상태)
      if (rc) {
         return rc;
      }
   }
   int moveCount = aNode->GetKeysNum(); //merge하여 현재 노드로 key값을 옮겼으므로 aNode에 있던 key값들을 삭제해야한다
   ...
   if (page == aNode->GetLeft()) { //현재 노드가 merge할 노드의 왼쪽에 위치할 경우
      SetRight(aNode->GetRight()); //현재 노드의 right을 aNode가 가리키던 right을 가리키게한다
   } else
      SetLeft(aNode->GetLeft()); //그반대
   return OK_RC;
}
...
// ************** my code start **************** //
RC IX_BTNode::Distribution(IX_BTNode * aNode){
    // exception
    if (1 + aNode->GetKeysNum() > order) { 
       return -1;
    }
        
    // insert smallest key at sibling
    void *k;
    this->GetKey(0, k); 
    RID rid = this->GetAddr(0);
    aNode->InsertNode(k, rid); 
    
    // delete smallest key at this node
    void * keytodel;
    GetKey(0, keytodel);
    RC rc = DeleteNode(keytodel, -1);
    if (rc == -1) return -1;
