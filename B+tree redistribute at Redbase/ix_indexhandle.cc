...
RC IX_IndexHandle::InsertEntry(void *pData, const RID &rid, int detail) {
    ...
    // ***************************** my code start ********************************//
    int isOK_Distribution = false;		// distribution possible flag
    int result_distribute = -1;			// distribution result
	...
	// internal node case   
        if(rootIsLeaf != true){
            IX_BTNode * parent = path[level];					// get parent node
            int pos = parent->FindKeyExact((const void*&) pData, rid);		// this(self) node position
            
	    // this node have sibling
            if(pos > 0){ 
                isOK_Distribution = true;					
		int posAtSibling = pos -1;					// sibling page num
		IX_BTNode* sibling = FetchNode(parent->rids[posAtSibling]);	// sibling node
                result_distribute = node->Distribution(sibling);		// distribution result
		int result2 = node->InsertNode(pData,rid);			// reinsert data
		void* key;

		node->GetKey(0, key);						
		if(result_distribute != -1){					// parent node key update
			parent->SetKey(posAtSibling, key);
		}
            }
        }
    }
    ...
    // sibling full -> split
    if(result_distribute == -1){ 
        isOK_Distribution = false;
    }
   
    // add distribution result flag
    while (result == -1 && isOK_Distribution == false) {	
	// ********************************* my code end **********************************//
